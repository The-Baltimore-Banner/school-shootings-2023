...
shootings <- victims %>%
  ...
  group_by(crime_date_time, location, district, neighborhood, description, race, weapon, premise, year, month, date, time_range, during_or_just_after_school, around_lunch, quarter, longitude, latitude) %>%
  summarise(victims = n(),
...
near_schools(3, final_hs_parcels$geometry[3])

```

## checking between 4 and 6 pm
```{r}
...
near_school %>%
  ...
  mutate(year = year(crime_date_time),
         month = month(crime_date_time),
         hour = hour(crime_date_time)) %>%
  mutate(bw_4and6 = case_when(
    hour >= 16 & hour <= 18 ~ TRUE,
    TRUE ~ FALSE
  )) %>%
  filter(bw_4and6 == TRUE)
